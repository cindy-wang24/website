...
// --- Extract landmarks & analysis functions ---
function extractKeyLandmarks(landmarks) {
  const keypoints = {nose:0,wrist_l:15,wrist_r:16,shoulder_l:11,shoulder_r:12,l_elbow:13,r_elbow:14,hip_l:23,hip_r:24,knee_l:25,knee_r:26,ankle_l:27,ankle_r:28};
  const extracted = {};
  for (let [k,i] of Object.entries(keypoints)) extracted[k]=[landmarks[i].x,landmarks[i].y];
  return extracted;
}
